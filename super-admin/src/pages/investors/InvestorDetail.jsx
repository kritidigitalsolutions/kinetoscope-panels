...
export default function InvestorDetail() {
  ...
  const [localRoiPercentage, setLocalRoiPercentage] = useState(investor ? (investor.roiPercentage || 1.2) : 1.2);
  const [showRoiEditModal, setShowRoiEditModal] = useState(false);
  const [roiEditStep, setRoiEditStep] = useState(1);
  const [roiInputVal, setRoiInputVal] = useState(investor ? String(investor.roiPercentage) : '1.2');

  const handleConfirmRoiChange = () => {
            ...
            <div className="kfpl-table-toolbar" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '16px 20px', borderBottom: '1px solid var(--color-border)', flexWrap: 'wrap', gap: '12px' }}>
              ...
              <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                ...
                <button
                  ...
                  onClick={() => {
                    downloadAllClientROICSV(investor.roiHistory, investor);
                ...
                <button
                  className="kfpl-btn kfpl-btn--ghost kfpl-btn--sm"
                  onClick={() => {
                    downloadAllClientROIPDF(investor.roiHistory, investor);
                  ...
                  {investor.roiHistory.length === 0 ? (
                    ...
                    investor.roiHistory.map(roi => (
                      <tr key={roi.id}>
                        ...
                        <td><strong>{roi.roiPercentage || localRoiPercentage || 1.2}%</strong></td>
                        ...
                        <td style={{ textAlign: 'center' }}>
                          <div style={{ display: 'inline-flex', gap: '6px', justifyContent: 'center' }}>
                            <button
                              className="kfpl-btn kfpl-btn--ghost kfpl-btn--sm"
                              onClick={() => {
                                downloadClientROISingleCSV(roi, investor);
                            ...
                            <button
                              className="kfpl-btn kfpl-btn--ghost kfpl-btn--sm"
                              onClick={() => {
                                downloadClientROISinglePDF(roi, investor);
